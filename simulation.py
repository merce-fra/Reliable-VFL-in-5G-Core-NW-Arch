...
for i in range(0,n_run):
    partitions, partitions_test, label, label_test, n , reliability = get_partitions_and_label(params,i,device)
    ...
    def server_fn(context: Context):
        server_config = ServerConfig(num_rounds=n_rounds)
        strategy = Strategy_test(label, label_test, i,n,scaled_latent_dims,optimized,n_rounds,device)
        return ServerAppComponents(
            strategy=strategy,
            config=server_config,
        )
